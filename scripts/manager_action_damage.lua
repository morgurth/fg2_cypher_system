...
function applyDamage(rSource, rTarget, bSecret, sRollType, sDamage, nTotal)
	...
	local _,sOriginalStatus = ActorManager2.getPercentWounded(sTargetType, nodeTarget);
	...
	if bShowStatus then
		local _,sNewStatus = ActorManager2.getPercentWounded(sTargetType, nodeTarget);
